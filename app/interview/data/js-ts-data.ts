...
export const JS_TS_DATA: QAItem[] = [
  ...
  // === JavaScript: Kiểu Dữ Liệu & Biến (201-210) ===
  {
    ...
    a: "- **var:** Function scope, hoisting (giá trị khởi tạo là `undefined`), có thể khai báo lại — dễ gây bug, tránh dùng.\n- **let:** Block scope, Temporal Dead Zone (TDZ — không thể truy cập trước khai báo), có thể gán lại.\n- **const:** Block scope, TDZ, KHÔNG THỂ gán lại — nhưng object/array bên trong vẫn mutable.\n\n**Best practice:** Mặc định dùng `const`, chỉ dùng `let` khi cần reassign, không bao giờ dùng `var`.",
    q_en: "What is the difference between var, let, and const?",
    a_en: "- **var:** Function scope, hoisting (value is `undefined`), can be re-declared — prone to bugs, avoid using it.\n- **let:** Block scope, Temporal Dead Zone (TDZ — cannot access before declaration), can be reassigned.\n- **const:** Block scope, TDZ, CANNOT be reassigned — but objects/arrays inside are still mutable.\n\n**Best practice:** Use `const` by default, only use `let` when reassignment is needed, never use `var`.",
  },
  {
    id: 202,
    category: "JavaScript",
  ...
  },
  {
    ...
    a: "Falsy values là các giá trị bị chuyển thành `false` trong ngữ cảnh boolean, gồm đúng 8 giá trị:\n\n- `false`\n- `0`\n- `-0`\n- `0n` (BigInt)\n- `''` (chuỗi rỗng)\n- `null`\n- `undefined`\n- `NaN`\n\nTất cả giá trị còn lại đều truthy — kể cả những giá trị dễ nhầm như `'0'` (string), `[]` (mảng rỗng), `{}` (object rỗng).\n\n**Bẫy phổ biến:** `if (arr.length)` hoạt động vì 0 là falsy, nhưng `if (obj)` luôn true dù obj là object rỗng. Dùng `Boolean(val)` hoặc `!!val` để chuyển về boolean tường minh.",
    q_en: "What are truthy and falsy values in JavaScript?",
    a_en: "Falsy values are those that convert to `false` in a boolean context, exactly 8 values:\n\n- `false`\n- `0`\n- `-0`\n- `0n` (BigInt)\n- `''` (empty string)\n- `null`\n- `undefined`\n- `NaN`\n\nAll remaining values are truthy — including easily confused ones like `'0'` (string), `[]` (empty array), `{}` (empty object).\n\n**Common trap:** `if (arr.length)` works because 0 is falsy, but `if (obj)` is always true even for an empty object. Use `Boolean(val)` or `!!val` for explicit boolean conversion.",
  },
  {
    id: 210,
    category: "JavaScript",
