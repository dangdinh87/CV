...
Differences from function constructors: classes cannot be called without \`new\` (throws TypeError), methods are non-enumerable (do not appear in for...in), and they must be declared before use (not hoisted). Under the hood they still use prototype chains — classes are just syntactic sugar.`,
  ...
  },
  {
    ...
    subcategory: "ES6+",
    level: "advanced",
    q: "WeakRef và FinalizationRegistry trong JavaScript là gì?",
    a: "**WeakRef** cho phép giữ một tham chiếu yếu (weak reference) đến một đối tượng mà không ngăn chặn đối tượng đó bị Garbage Collector thu hồi. Hữu ích cho việc cache dữ liệu lớn hoặc DOM nodes. Để đọc đối tượng, dùng `deref()`.\n\n**FinalizationRegistry** cho phép đăng ký một callback để thực thi sau khi một đối tượng đã bị Garbage Collected. Dùng để cleanup tài nguyên hoặc log memory leak.\n\n- **Lưu ý quan trọng:** Cả hai tính năng này nên được dùng cẩn thận và tránh phụ thuộc vào hành vi Garbage Collection vì chúng không đảm bảo thời gian chạy cụ thể.",
    q_en: "What are WeakRef and FinalizationRegistry in JavaScript?",
    a_en: "**WeakRef** allows holding a weak reference to an object without preventing it from being garbage-collected. Useful for caching large data or DOM nodes. To read the object, use `deref()`.\n\n**FinalizationRegistry** lets you register a callback to execute after an object has been garbage-collected. Used for resource cleanup or logging memory leaks.\n\n- **Important note:** Both features should be used carefully and you should avoid relying on Garbage Collection behavior since there are no guarantees about when it runs.",
  },
