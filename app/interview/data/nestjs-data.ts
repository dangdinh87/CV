...
export const NESTJS_DATA: QAItem[] = [
  {
    ...
    a: "NestJS là framework Node.js xây dựng trên TypeScript, lấy cảm hứng từ Angular. Nó cung cấp kiến trúc module rõ ràng với `@Module()`, `@Controller()`, `@Injectable()` và Dependency Injection tích hợp sẵn.\n\n**Lý do chọn NestJS thay Express thuần:**\n- **Kiến trúc có cấu trúc:** Modules, Controllers, Services phân tách rõ ràng.\n- **TypeScript-first:** Hỗ trợ tốt decorators và metadata reflection.\n- **IoC container:** Tích hợp sẵn giúp Dependency Injection và testing dễ dàng.\n- **Đa dụng:** Hỗ trợ Microservices, GraphQL, WebSockets, gRPC.\n\nNestJS vẫn chạy trên Express (hoặc Fastify) bên dưới, nhưng thêm lớp abstraction giúp code dễ maintain và scale hơn.",
  ...
  },
  {
    ...
    subcategory: "Interceptors",
    level: "intermediate",
    q: "Interceptor trong NestJS là gì và trường hợp nào nên sử dụng?",
    a: "Interceptor cho phép chặn (intercept) vòng đời của một request. Nó có thể:\n- **Chạy logic trước/sau** khi route handler được gọi.\n- **Biến đổi kết quả (Transform)** trước khi trả về cho client.\n- **Xử lý ngoại lệ (Exception Mapping)**.\n- **Override hành vi mặc định**, ví dụ như caching kết quả.\n\nInterceptor hoạt động dưa trên RxJS (Reactive Extensions for JS), nên thường xuyên làm việc với Observable. \n\n**Ví dụ sử dụng:** Logging request/response, tự động serialize object thành JSON tiêu chuẩn, hoặc cache data.",
    q_en: "What is an Interceptor in NestJS and when should you use it?",
    a_en: "Interceptors allow you to hook into the request/response lifecycle. They can:\n- **Execute logic before/after** a route handler is called.\n- **Transform results** before sending them to the client.\n- **Map exceptions** to customize error output.\n- **Override default behaviors**, like caching results.\n\nInterceptors rely heavily on RxJS (Reactive Extensions for JS) and typically use Observables.\n\n**Use cases:** Logging requests/responses, standardizing JSON output formats, or caching data.",
  },
];
