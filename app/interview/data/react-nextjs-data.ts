...
export const REACT_NEXTJS_DATA: QAItem[] = [
  ...
  },
  {
    ...
    a: `Có nhiều cách:\n\n- Toán tử ba ngôi: \`isLoggedIn ? <Dashboard /> : <Login />\`\n- Toán tử &&: \`isLoading && <Spinner />\`\n- Hoặc \`if/else\` bên ngoài return.\n\nTrong thực tế, ternary dùng cho toggle giữa 2 component, && dùng để hiển thị/ẩn một component.\n\n**Lưu ý:** Cần tránh \`count && <List />\` vì nếu count=0, React render số 0 thay vì không render gì.`,
    q_en: "How is conditional rendering handled in React?",
    a_en: `There are several approaches:\n\n- The ternary operator: \`isLoggedIn ? <Dashboard /> : <Login />\`\n- The logical AND: \`isLoading && <Spinner />\`\n- Or an \`if/else\` block outside the return statement.\n\nIn practice, ternary is used to toggle between two components, and && is used to show or hide a single component.\n\n**Pitfall:** Avoid \`count && <List />\` because if count is 0, React renders the number 0 instead of nothing.`,
  },
  {
    id: 404,
    category: "React",
...
\`\`\``,
  ...
  // === React - React 19 (601-610) ===
  {
    id: 601,
    category: "React",
    subcategory: "React 19",
    level: "intermediate",
    q: "Hook `use` trong React 19 là gì?",
    a: "Trong React 19, `use` là một API mới dùng để đọc giá trị của các resource bất đồng bộ như Promise hoặc Context. Khác với các Hook thông thường (như `useContext`), `use` có thể được gọi linh hoạt bên trong các vòng lặp, câu lệnh điều kiện (if/else), hoặc các block return sớm. Khi truyền vào một Promise, `use` tích hợp sẵn với Suspense: nó sẽ tạm dừng (suspend) việc render component cho đến khi Promise giải quyết xong, giúp đơn giản hoá luồng xử lý dữ liệu bất đồng bộ.",
    q_en: "What is the `use` API in React 19?",
    a_en: "In React 19, `use` is a new API used to read the value of an asynchronous resource such as a Promise or Context. Unlike standard Hooks (like `useContext`), `use` can be called conditionally within loops, if/else statements, and early returns. When passed a Promise, it integrates seamlessly with Suspense: it suspends the component's rendering until the Promise resolves, which significantly simplifies asynchronous data fetching.",
  },
];
