...
export const REACT_NEXTJS_DATA: QAItem[] = [
  // === React - JSX & Cơ Bản (401-415) ===
  {
    ...
    a: "JSX (JavaScript XML) là cú pháp mở rộng của JavaScript cho phép viết HTML bên trong JavaScript. React sử dụng JSX vì:\n\n- **Trực quan hơn:** Giúp mô tả UI dễ dàng như viết HTML.\n- **Component-based:** Kết hợp logic và markup trong cùng một file component thay vì tách rời.\n- **Transpile:** Babel transpile JSX thành các lời gọi `React.createElement()` trước khi chạy trên trình duyệt.",
    q_en: "What is JSX and why does React use it?",
    a_en: "JSX (JavaScript XML) is a syntax extension for JavaScript that lets you write HTML-like markup inside JavaScript. React uses JSX because:\n\n- **More intuitive:** Makes describing UI as easy as writing HTML.\n- **Component-based:** Combines logic and markup in the same component file instead of separating them.\n- **Transpilation:** Babel transpiles JSX into `React.createElement()` calls before the code runs in the browser.",
  },
  {
    id: 402,
    category: "React",
...
\`\`\``,
  ...
  },
  {
    ...
    subcategory: "Advanced Hooks",
    level: "advanced",
    q: "useActionState hook (React 19) là gì và khác gì với useFormState?",
    a: "useActionState là hook mới trong React 19 (trước đây gọi là useFormState trong React DOM) dùng để quản lý state của form action, trả về mảng `[state, action, isPending]`. Nó tự động handle pending state và error state từ action, thay thế pattern thủ công quản lý loading/error. Tích hợp sâu với Server Actions trong Next.js.",
    q_en: "What is the useActionState hook in React 19 and how does it differ from useFormState?",
    a_en: "useActionState is a new hook in React 19 (previously known as useFormState in React DOM) that manages the state of a form action, returning `[state, action, isPending]`. It automatically handles the pending and error states returned from the action, replacing manual loading/error state management. It integrates deeply with Server Actions in Next.js.",
  },
];
