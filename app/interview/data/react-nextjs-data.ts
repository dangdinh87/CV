...
export const REACT_NEXTJS_DATA: QAItem[] = [
  ...
  },
  {
    ...
    a: "props.children là nội dung được truyền giữa thẻ mở và đóng của component.\n- **Ví dụ:** `<Card><p>Nội dung</p></Card>` thì Card nhận `<p>Nội dung</p>` qua props.children và render bằng `{props.children}`.\n- Dùng để tạo wrapper component linh hoạt như Card, Modal, Layout mà không cần biết trước nội dung bên trong.\n- Nếu không truyền gì, children là undefined — nên kiểm tra trước khi render hoặc gán default.",
    q_en: "What is props.children and how do you use it?",
    a_en: "props.children is the content passed between a component's opening and closing tags.\n- **Example:** in `<Card><p>Content</p></Card>`, the Card component receives `<p>Content</p>` via props.children and renders it with `{props.children}`.\n- Use it to build flexible wrapper components like Card, Modal, or Layout without needing to know the inner content in advance.\n- If nothing is passed, children is undefined — check for it or provide a default before rendering.",
  },
  {
    id: 418,
    category: "React",
    ...
    a: "- **Function Component:** Khai báo default values trực tiếp trong destructuring: `function Button({ color = 'blue', size = 'md' }) {}` — cách ngắn gọn và được khuyến nghị.\n- **Class Component:** Dùng `Button.defaultProps = { color: 'blue' }` sau khai báo class.\n- **TypeScript:** Định nghĩa interface cho props rồi đặt default trong destructuring giúp vừa type-safe vừa có default.\n- **Lưu ý:** `defaultProps` trong function component đã bị deprecate từ React 18.3.",
    q_en: "How do you declare default props in React?",
    a_en: "- **Function Component:** Declare default values directly in the destructuring parameters: `function Button({ color = 'blue', size = 'md' }) {}` — this is the concise, recommended approach.\n- **Class Component:** Use `Button.defaultProps = { color: 'blue' }` after the class declaration.\n- **TypeScript:** Define an interface for props and set defaults in destructuring for both type safety and default values.\n- **Note:** `defaultProps` on function components was deprecated in React 18.3.",
  },
  {
    id: 419,
    category: "React",
...
Understanding this algorithm helps you avoid bugs and write more performant component trees.`,
...

  {
    ...
    subcategory: "Performance & Patterns",
    level: "advanced",
    q: "React Compiler (React 19) là gì và nó giải quyết vấn đề gì?",
    a: "React Compiler (trước đây là React Forget) là một trình biên dịch (compiler) tự động thêm quá trình memoization vào code React trong build-time.\n\n- **Vấn đề giải quyết:** Trước đây, developer phải tự dùng `useMemo`, `useCallback`, và `React.memo` để tối ưu hiệu suất, tránh re-render không cần thiết. Việc này dễ sai sót và làm code khó đọc.\n- **Cách hoạt động:** Nó phân tích mã nguồn và tự động tạo các logic ghi nhớ (memoization) dưới hook để chỉ re-render những phần UI có props hoặc state thay đổi.\n- **Lợi ích:** Developer có thể tập trung vào logic tính năng thay vì phải tối ưu thủ công, giúp code gọn gàng hơn và ứng dụng tự nhiên mượt mà hơn.",
    q_en: "What is the React Compiler (React 19) and what problem does it solve?",
    a_en: "React Compiler (formerly React Forget) is a build-time compiler that automatically adds memoization to your React code.\n\n- **Problem solved:** Previously, developers had to manually use `useMemo`, `useCallback`, and `React.memo` to optimize performance and prevent unnecessary re-renders. This was error-prone and cluttered the codebase.\n- **How it works:** It analyzes the source code and automatically generates memoization logic under the hood so that only UI parts with changed props or state are re-rendered.\n- **Benefits:** Developers can focus on feature logic instead of manual optimization, resulting in cleaner code and apps that are naturally smoother.",
  },
