...
export const REACT_NEXTJS_DATA: QAItem[] = [
  // === React - JSX & Cơ Bản (401-415) ===
  {
    ...
    a: "**JSX** (JavaScript XML) là cú pháp mở rộng của JavaScript cho phép viết HTML bên trong JavaScript.\n\nReact sử dụng JSX vì nó giúp mô tả UI trực quan hơn, kết hợp logic và markup trong cùng một file component.\n\nBabel transpile JSX thành các lời gọi `React.createElement()` trước khi chạy trên trình duyệt.\n\n**Ví dụ:**\n```jsx\nconst element = <h1>Hello, world!</h1>;\n```",
    q_en: "What is JSX and why does React use it?",
    a_en: "**JSX** (JavaScript XML) is a syntax extension for JavaScript that lets you write HTML-like markup inside JavaScript.\n\nReact uses JSX because it makes describing UI more intuitive by combining logic and markup in the same component file.\n\nBabel transpiles JSX into `React.createElement()` calls before the code runs in the browser.\n\n**Example:**\n```jsx\nconst element = <h1>Hello, world!</h1>;\n```",
  },
  {
    id: 402,
    category: "React",
...
\`\`\``,
  ...
  },
  {
    ...
    level: "advanced",
    q: "use() hook trong React 19 là gì và dùng khi nào?",
    a: 'use() là một hook mới trong React 19 dùng để đọc value của một Promise hoặc Context bên trong render phase. Điểm đặc biệt của use() so với các hook khác (như useContext) là nó có thể được gọi bên trong các khối điều kiện (if) hoặc vòng lặp. Khi truyền Promise vào use(), nó tích hợp sẵn với Suspense: component sẽ "suspend" (chờ) cho đến khi Promise resolve, giúp code lấy dữ liệu không đồng bộ trông giống như đồng bộ và dễ đọc hơn.',
    q_en: "What is the use() hook in React 19 and when is it used?",
    a_en: 'use() is a new hook in React 19 used to read the value of a Promise or Context during the render phase. A unique feature of use() compared to other hooks (like useContext) is that it can be called inside conditional blocks (if) or loops. When passing a Promise to use(), it integrates with Suspense: the component will "suspend" (wait) until the Promise resolves, making asynchronous data fetching code look synchronous and easier to read.',
  },
];
