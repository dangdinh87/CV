...
\`\`\``,
  },
  {
    ...
    a: "Virtual DOM là representation (bản sao) trong memory của DOM thực.\n\n**Quy trình hoạt động:**\n- Khi state thay đổi, React tạo Virtual DOM mới.\n- So sánh (diff) với Virtual DOM cũ.\n- Tính ra minimal set of changes (các thay đổi tối thiểu).\n- Áp dụng vào DOM thực (quá trình này gọi là reconciliation).\n\n**Lợi ích:** Giúp tối ưu DOM operations tốn kém, nhưng không phải lúc nào cũng nhanh hơn direct DOM manipulation.",
    q_en: "What is the Virtual DOM in React?",
    a_en: "The Virtual DOM is an in-memory representation of the real DOM.\n\n**How it works:**\n- When state changes, React creates a new Virtual DOM tree.\n- It diffs it against the previous one.\n- Computes the minimal set of changes.\n- Applies them to the real DOM (reconciliation).\n\n**Benefits:** This optimizes expensive DOM operations, though it is not always faster than direct DOM manipulation for simple cases.",
  },
  {
    id: 539,
    category: "React",
...
\`\`\``,
  ...
  },
  {
    ...
    subcategory: "Next.js",
    level: "intermediate",
    q: "Sự khác biệt giữa React Server Components (RSC) và Client Components là gì?",
    a: "**React Server Components (RSC):**\n- Render trên server và không gửi JS xuống client (zero bundle size).\n- Có thể truy cập trực tiếp backend resources (database, file system).\n- Không thể dùng hooks như `useState`, `useEffect` hoặc event listeners (onClick).\n\n**Client Components:**\n- Khai báo bằng `'use client'` directive ở đầu file.\n- Render trên client (có hydration).\n- Hỗ trợ đầy đủ hooks và tính năng tương tác của React.\n\n**Cách dùng:** Mặc định trong Next.js App Router mọi component là RSC. Chỉ dùng Client Component khi cần tương tác từ người dùng (interactivity) hoặc sử dụng hooks của React.",
    q_en: "What is the difference between React Server Components (RSC) and Client Components?",
    a_en: "**React Server Components (RSC):**\n- Rendered on the server and send no JS to the client (zero bundle size).\n- Can directly access backend resources like databases and file systems.\n- Cannot use hooks like `useState`, `useEffect`, or event listeners (onClick).\n\n**Client Components:**\n- Declared using the `'use client'` directive at the top of the file.\n- Rendered on the client (with hydration).\n- Fully support React hooks and interactivity.\n\n**Usage:** In Next.js App Router, components are RSC by default. Only use Client Components when user interactivity or React hooks are needed.",
  },
];
