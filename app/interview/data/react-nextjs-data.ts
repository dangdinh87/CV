...
\`\`\`` },
  ...
  { id: 444, category: "React", subcategory: "Props & State", level: "intermediate", q: "Controlled vs Uncontrolled components khác nhau như thế nào?", a: `- Controlled component: giá trị form được kiểm soát bởi React state, phải có onChange handler, phù hợp khi cần validation hay transform dữ liệu.
- Uncontrolled component: giá trị lưu trong DOM, dùng ref để đọc, đơn giản hơn cho form không cần real-time validation.

React khuyến nghị Controlled trong hầu hết trường hợp.`, q_en: "How do Controlled and Uncontrolled components differ?", a_en: `- Controlled component: the form value is managed by React state, requires an onChange handler, and is suited for validation or data transformation.
...
React recommends the Controlled approach in most cases.` },
  ...
  { id: 453, category: "React", subcategory: "Lifecycle", level: "intermediate", q: "Thứ tự lifecycle methods khi component mount, update, unmount là gì?", a: `Thứ tự lifecycle methods qua 3 giai đoạn:
...
- Với concurrent mode, render phase có thể bị interrupt.`, q_en: "What is the order of lifecycle methods during mount, update, and unmount?", a_en: `Lifecycle methods go through 3 phases:
...
\`\`\`` },
  ...
  { id: 488, category: "React", subcategory: "Advanced Hooks", level: "intermediate", q: "Quy tắc của Hooks (Rules of Hooks) là gì?", a: `Hai quy tắc bắt buộc:
...
Vi phạm gây lỗi runtime khó debug. Cài \`eslint-plugin-react-hooks\` để bắt lỗi ngay lúc viết code thay vì lúc chạy.`, q_en: "What are the Rules of Hooks?", a_en: `Two mandatory rules:
...
\`\`\`` },
  ...
  { id: 505, category: "React", subcategory: "Custom Hooks", level: "advanced", q: "Patterns hay được dùng trong custom hooks là gì?", a: `Các patterns phổ biến:
...
usehooks.com là nguồn tham khảo tốt.`, q_en: "What patterns are commonly used when building custom hooks?", a_en: `Common patterns:
...
\`\`\`` },
  ...
  { id: 525, category: "React", subcategory: "Forms & Error", level: "intermediate", q: "Form validation trong React được thực hiện như thế nào?", a: `Các cách:
...
Dùng Zod cho type-safe validation kết hợp TypeScript là best practice hiện đại.`, q_en: "How is form validation done in React?", a_en: `Several approaches:
...
Using Zod for type-safe validation combined with TypeScript is the modern best practice.` },
  ...
  { id: 531, category: "React", subcategory: "Forms & Error", level: "beginner", q: "Làm thế nào để handle select, checkbox, radio trong React?", a: `- Select: value bind với state, onChange cập nhật.
...
Controlled pattern áp dụng tương tự cho tất cả.`, q_en: "How do you handle select, checkbox, and radio inputs in React?", a_en: `- Select: bind value to state and update in onChange.
...
\`\`\`` },
  ...
  { id: 542, category: "React", subcategory: "Performance & Patterns", level: "advanced", q: "Reconciliation algorithm trong React hoạt động như thế nào?", a: `React dùng heuristic O(n) algorithm thay vì O(n³) để diff trees:
...
Hiểu algorithm giúp tránh bugs và tối ưu performance.`, q_en: "How does React's reconciliation algorithm work?", a_en: `React uses an O(n) heuristic algorithm instead of the theoretically optimal O(n³) to diff trees:
...
\`\`\`` },
  ...
  { id: 573, category: "Next.js", subcategory: "Rendering", level: "intermediate", q: "ISR (Incremental Static Regeneration) là gì?", a: `ISR kết hợp SSG và SSR: trang được generate tĩnh, nhưng sau \`revalidate\` seconds có thể regenerate lại. User đầu tiên sau revalidate nhận stale content, trigger background regeneration; user sau nhận fresh content.

- Pages Router: \`return { revalidate: 60 }\` trong getStaticProps.
- App Router: \`export const revalidate = 60\` hoặc \`fetch(url, { next: { revalidate: 60 } })\`.`, q_en: "What is ISR (Incremental Static Regeneration)?", a_en: `ISR blends SSG and SSR: pages are statically generated but can be regenerated in the background after a \`revalidate\` interval. The first user after the interval receives stale content and triggers a background regeneration; subsequent users get the fresh version.

- Pages Router: \`return { revalidate: 60 }\` in getStaticProps.
- App Router: \`export const revalidate = 60\` or \`fetch(url, { next: { revalidate: 60 } })\`.` },
  { id: 574, category: "Next.js", subcategory: "Rendering", level: "intermediate", q: "On-demand revalidation trong Next.js là gì?", a: `On-demand revalidation cho phép invalidate cache và regenerate pages ngay lập tức thay vì đợi timeout.
...
Phù hợp khi CMS webhook trigger cần update content ngay.`, q_en: "What is on-demand revalidation in Next.js?", a_en: `On-demand revalidation lets you invalidate the cache and regenerate pages immediately rather than waiting for a timer.
...
Ideal when a CMS webhook fires and content needs to update right away.` },
  { id: 575, category: "Next.js", subcategory: "Rendering", level: "intermediate", q: "fetch cache trong Next.js App Router hoạt động như thế nào?", a: `Next.js extend native fetch với caching:
...
Data cache tách biệt với Full Route Cache.`, q_en: "How does fetch caching work in the Next.js App Router?", a_en: `Next.js extends the native fetch API with caching options:
...
The Data Cache is separate from the Full Route Cache.` },
  { id: 576, category: "Next.js", subcategory: "Rendering", level: "advanced", q: "Next.js có những loại cache nào?", a: `Next.js có 4 loại cache:
...
- (4) Router Cache: client-side cache của RSC payload, prefetch routes.`, q_en: "What types of caching does Next.js have?", a_en: `Next.js has 4 caching layers:
...
- (4) Router Cache — client-side cache of the RSC payload used for prefetching routes.` },
  { id: 577, category: "Next.js", subcategory: "Rendering", level: "intermediate", q: "Sự khác biệt giữa dynamic và static rendering trong App Router?", a: `- Static rendering: render lúc build time, cache và serve từ CDN.
- Dynamic rendering: render mỗi request, không cache.

Tự động dynamic khi dùng: \`await cookies()\`, \`await headers()\`, \`searchParams\`, hay fetch với \`cache: 'no-store'\`. Explicit: \`export const dynamic = 'force-dynamic'\` hoặc \`'force-static'\`. Opt-in static càng nhiều càng tốt cho performance.`, q_en: "What is the difference between dynamic and static rendering in the App Router?", a_en: `- Static rendering: happens at build time, cached and served from a CDN.
