...
export const REACT_NEXTJS_DATA: QAItem[] = [
  ...
  },
  {
    ...
    a: "Có nhiều cách khai báo default props tùy thuộc vào loại component:\n\n- **Function Component:** Khai báo trực tiếp trong destructuring (đây là cách được khuyến nghị):\n  `function Button({ color = 'blue', size = 'md' }) {}`\n- **Class Component:** Sử dụng `defaultProps` sau khi khai báo class:\n  `Button.defaultProps = { color: 'blue' }`\n- **TypeScript:** Định nghĩa interface cho props và thiết lập default ngay trong destructuring, giúp code type-safe và có mặc định rõ ràng.\n\n*Lưu ý:* `defaultProps` trong function component đã bị deprecate kể từ React 18.3.",
    q_en: "How do you declare default props in React?",
    a_en: "There are different ways to declare default props depending on the component type:\n\n- **Function Component:** Declare directly in the destructuring parameters (recommended approach):\n  `function Button({ color = 'blue', size = 'md' }) {}`\n- **Class Component:** Use `defaultProps` after the class declaration:\n  `Button.defaultProps = { color: 'blue' }`\n- **TypeScript:** Define an interface for the props and set defaults in destructuring. This ensures both type safety and clear default values.\n\n*Note:* `defaultProps` on function components was deprecated in React 18.3.",
  },
  {
    id: 419,
    category: "React",
  ...
  },
  {
    ...
    a: "**Component Composition** là kỹ thuật xây dựng các component phức tạp bằng cách kết hợp nhiều component đơn giản, nhỏ gọn lại với nhau thay vì dùng tính kế thừa (inheritance).\n\nReact đặc biệt khuyến khích sử dụng composition vì:\n- **Linh hoạt hơn:** Dễ dàng thay đổi và tái sử dụng UI.\n- **Dễ test hơn:** Các component nhỏ dễ dàng kiểm thử độc lập.\n- **Tránh rắc rối kế thừa:** Không gặp các vấn đề phức tạp liên quan đến prototype chain.\n\n*Ví dụ:* Các pattern như `children` prop, render props, và slots đều là những ứng dụng phổ biến của composition.",
    q_en: "What is component composition and why is it preferred over inheritance?",
    a_en: "**Component Composition** is the technique of building complex components by assembling simpler, smaller components together, rather than relying on class inheritance.\n\nReact strongly favors composition over inheritance because it is:\n- **More flexible:** Easier to change and reuse UI structures.\n- **Easier to test:** Small, isolated components are simpler to unit test.\n- **Avoids inheritance issues:** You don't run into the deep complexities of prototype chains.\n\n*Examples:* Patterns such as the `children` prop, render props, and slots are all common expressions of composition.",
  },
  {
    id: 422,
    category: "React",
